...
public class LIDAR {
    ...
    /*
     * public int LidarLitePWM(source) { counter = new Counter(source);
     * counter.setMaxPeriod(1.0); counter.setSemiPeriodMode(true); counter.reset();
     * }
     */
    ...
    public double getDistance() { // Method to find LIDAR distance reading at any given time
        ...
        // System.out.println(connection); // Returns true if the sensor was verified to
        // be connected
        // System.out.println(failed); // Returns false when we are getting actual
        // values from the arduino

        /*
         * Prints for Serial System.out.println(serialPort.read(2)[0]);
         * System.out.println(serialPort.read(2)[1]); System.out.println(bytes[0]);
        ...
        // int distance = java.nio.ByteBuffer.wrap(bytes).getInt(); // Convert the bytes
        // to an integer value
