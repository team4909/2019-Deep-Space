...
let clientDataListener = (key, val, valType, mesgType, id, flags) => {
    ...
    operatorWindow.webContents.send(mesgType, {
        key,
        val,
        valType,
        id,
...
function createWindow() {
    ...
    client.start((con, err) => {
        ...
        let connectFunc = () => {
            ...
            operatorWindow.webContents.send('connected', con);
    ...
    ipc.on('ready', (ev, mesg) => {
        ...
        ready = mainWindow != null && operatorWindow != null;
    ...
    // Move window to top (left) of screen.
    operatorWindow.setPosition(0, 0);
    // Load window.
    operatorWindow.loadURL(`file://${__dirname}/operator.html`);
    // Once the python server is ready, load window contents.
    operatorWindow.once('ready-to-show', () => {
        console.log('operator window is ready to be shown');
        operatorWindow.show();
    });

    operatorWindow.on('closed', () => {
        console.log('operator window closed');
        // Dereference the window object, usually you would store windows
        // in an array if your app supports multi windows, this is the time
        // when you should delete the corresponding element.
        operatorWindow = null;
    ...
    operatorWindow.on('unresponsive', () => {
        console.log('Operator Window is unresponsive');
    });
